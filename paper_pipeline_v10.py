#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""paper_pipeline_v10.py

[windows]
C:/Users/takumi/develop/miniconda3/python.exe APA/paper_pipeline_v10.py

[mac]
# リポジトリルートから実行する想定（`APA/` 配下のスクリプトを直接指定）
.venv/bin/python paper_pipeline_v10.py
...
4) フォーム判定（回転探索）
   ...
   - フォームA: 3点マーク（TL/TR/BL）が検出できる（`--marker-preproc` で前処理オプション）
     - v10 では **フォームAスコアが十分高い場合**、速度のため **フォームB判定をスキップ**する
   ...
   - フォームB: QRコードが検出できる
     ...
     - v10 では WeChat-only でも v7 と同様に **fast → robust の2段階**で評価する
...
※v10 ではデバッグ画像の保存量を `--save-images {all,fail,none}` で制御できる。
...
PIPELINE_DEFAULTS: dict[str, Any] = {
    ...
    "rotation_scan": {
        ...
        # v10.2 改善: rectify 後は enforce_landscape で横長に統一されているため、
...
class WeChatQRDetector:
    ...
    def detect(self, image_bgr: np.ndarray) -> list[dict[str, Any]]:
        ...
        # v10 改善: 呼び出し側で「detector をスレッド数分用意」し、ここでは Lock を使わない。
...
def init_wechat_qr_detector(
    ...
    *,
    ...
    global _WECHAT_QR
    try:
        # v10 改善: detector をスレッド数ぶん用意し、Lock による直列化を避ける。
...
# --- 既存実装の流用 ---
# 注意: このスクリプトは `python APA/paper_pipeline_v10.py ...` の形で実行される想定。
...
def score_formB_fast(image_bgr: np.ndarray) -> tuple[bool, float, dict[str, Any]]:
    ...
    v7の設計（fast→必要ならrobust）を v10(WeChat-only) にも導入するためのもの。
    - fast は見つけたら即返す
...
v10 ではユーザー要望により「フォーム確定後は全テンプレを XFeat で照合」します。
...
def select_top_templates(
    ...
    # v10 では prefilter を使わないため、互換用に「そのまま返す」だけにする。
...
def build_csv_row(
    *,
    ...
    row: dict[str, Any] = {
        ...
        # v10 では template-topn は廃止（常に全テンプレ照合）
...
def print_explain() -> None:
    ...
    lines = [
        ...
        "  (注) v10 ではテンプレ候補絞り込み（template-topn）は廃止し、常に全テンプレ照合します。",
        "",
        "【ログ】",
        ...
        "",
        "最小コマンド例（おすすめデフォルト使用）:",
        r"  C:\Users\takumi\develop\miniconda3\python.exe APA\paper_pipeline_v10.py --limit 1",
        "",
    ]
    print("\n".join(lines))
...
def main(argv=None) -> int:
    ...
    logger.info("=" * 70)
    logger.info("paper_pipeline_v10")
    logger.info("=" * 70)
    ...
    # v10 改善: 回転スキャンでの直列化を避けるため、ThreadPool の worker 数だけ detector を確保する。
