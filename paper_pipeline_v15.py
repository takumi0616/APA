...
4) フォーム判定（回転探索）
   ...
   - フォームB: QRコードが検出できる
     ...
     - v15 では WeChat-only でも v7 と同様に **fast → robust の2段階**で評価する
...
※v15 ではデバッグ画像の保存量を `--save-images {all,fail,none}` で制御できる。
...
PIPELINE_DEFAULTS: dict[str, Any] = {
    ...
    "rotation_scan": {
        ...
        # v15.2 改善: rectify 後は enforce_landscape で横長に統一されているため、
...
class WeChatQRDetector:
    ...
    def detect(self, image_bgr: np.ndarray) -> list[dict[str, Any]]:
        ...
        # v15 改善: 呼び出し側で「detector をスレッド数分用意」し、ここでは Lock を使わない。
...
def init_wechat_qr_detector(
    ...
    *,
    ...
    global _WECHAT_QR
    try:
        # v15 改善: detector をスレッド数ぶん用意し、Lock による直列化を避ける。
...
def score_formB_fast(image_bgr: np.ndarray) -> tuple[bool, float, dict[str, Any]]:
    ...
    v7の設計（fast→必要ならrobust）を v15(WeChat-only) にも導入するためのもの。
    - fast は見つけたら即返す
...
v15 ではユーザー要望により「フォーム確定後は全テンプレを XFeat で照合」します。
...
def select_top_templates(
    ...
    # v15 では prefilter を使わないため、互換用に「そのまま返す」だけにする。
...
def build_csv_row(
    *,
    ...
    row: dict[str, Any] = {
        ...
        # v15 では template-topn は廃止（常に全テンプレ照合）
...
def print_explain() -> None:
    ...
    lines = [
        ...
        "  (注) v15 ではテンプレ候補絞り込み（template-topn）は廃止し、常に全テンプレ照合します。",
        "",
        "【ログ】",
        ...
        "",
        "最小コマンド例（おすすめデフォルト使用）:",
        r"  C:\Users\takumi\develop\miniconda3\python.exe APA\paper_pipeline_v15.py --limit 1",
        "",
    ]
    print("\n".join(lines))
...
def _apply_extra_degrade_v15_5(
    *,
    src_bgr: np.ndarray,
    degraded_bgr: np.ndarray,
    ...
    """v15.5 の追加改悪（bend/shadow）を適用する。

    NOTE:
      v15.7 では「改悪生成フェーズ」を最初に全件実行するため、
      追加改悪もここでまとめて適用する。
    """
    ...
    degrade_meta["extra_degrade_v15_5"] = True
...
def process_one_case(
    *,
    ...
    """改悪生成済み画像（degraded_input）を処理する（v15.7）。
...
def process_one_observed_case(
    *,
    ...
    # v15.7 方針: 改悪生成以外でも「画像保存等の周辺処理」は時間計測に含めない。
    ...
    warped_final = cv2.warpPerspective(chosen_for_match, H_img_to_tpl, (tpl_bgr.shape[1], tpl_bgr.shape[0]))
    out_aligned = out_dirs["aligned"] / f"{case_id}_aligned.jpg"
    # v15.7: aligned は成果物なので save-images に関係なく必ず保存し、保存時間も計測に含める。
    write_image(out_aligned, warped_final, jpeg_quality=jpeg_quality)
    item["output_aligned_image_path"] = str(out_aligned)
    times.warp_s = time.perf_counter() - t0
    item["ok_warp"] = True
...
def main(argv=None) -> int:
    ...
    logger.info("=" * 70)
    logger.info("paper_pipeline_v15")
    logger.info("=" * 70)
    ...
    # v15 改善: 回転スキャンでの直列化を避けるため、ThreadPool の worker 数だけ detector を確保する。
    ...
    def _generate_one(
        *,
        ...
        # v15.5 extra degrade (bend/shadow)
        try:
            degraded_bgr, degrade_meta = _apply_extra_degrade_v15_5(
                src_bgr=src_bgr,
                degraded_bgr=degraded_bgr,
            ...
            if isinstance(degrade_meta, dict):
                degrade_meta.setdefault("extra_degrade_v15_5", False)
