...
class WeChatQRDetector:
    ...
    def detect(self, image_bgr: np.ndarray) -> list[dict[str, Any]]:
        ...
        # v6 改善: 呼び出し側で「detector をスレッド数分用意」し、ここでは Lock を使わない。
...
def init_wechat_qr_detector(
    ...
    *,
    ...
    global _WECHAT_QR
    try:
        # v6 改善: detector をスレッド数ぶん用意し、Lock による直列化を避ける。
...
# --- 既存実装の流用 ---
# 注意: このスクリプトは `python APA/paper_pipeline_v6.py ...` の形で実行される想定。
...
v6 ではユーザー要望により「フォーム確定後は全テンプレを XFeat で照合」します。
...
def select_top_templates(
    ...
    # v6 では prefilter を使わないため、互換用に「そのまま返す」だけにする。
...
def build_csv_row(
    *,
    ...
    row: dict[str, Any] = {
        ...
        # v6 では template-topn は廃止（常に全テンプレ照合）
...
def print_explain() -> None:
    ...
    lines = [
        ...
        "  (注) v6 ではテンプレ候補絞り込み（template-topn）は廃止し、常に全テンプレ照合します。",
        "",
        "【ログ】",
        ...
        "",
        "最小コマンド例（おすすめデフォルト使用）:",
        r"  C:\Users\takumi\develop\miniconda3\python.exe APA\paper_pipeline_v6.py --limit 1",
        "",
    ]
    print("\n".join(lines))
...
def main(argv=None) -> int:
    ...
    # v6 改善: 回転スキャンでの直列化を避けるため、ThreadPool の worker 数だけ detector を確保する。
